...
export async function POST(request: Request, { params }: { params: { request_id: string } }) {
  try {
    ...
    try {
      ...
      // Find a random master
      const masters = await db.collection("users").find({ role: "master" }).toArray()

      if (masters.length === 0) {
        return NextResponse.json({ detail: "No masters available" }, { status: 400 })
      }

      // Select a random master
      ...
      const result = await db.collection("support_requests").updateOne(
        ...
        {
          $set: {
            assigned_master_id: randomMaster._id.toString(),
            status: "approved",
            updated_at: new Date(),
          },
        },
      ...
      return NextResponse.json(
        {
          ...
          master_id: randomMaster._id.toString(),
          master_name: `${randomMaster.first_name} ${randomMaster.last_name}`,
        },
        { status: 200 },
      )
    } catch (error) {
